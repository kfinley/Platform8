...
namespace Platform8.Budgets.Tests {
  ...
  public class When_AddCategory_Requested : SpecBase {
    ...
    Establish context = () => {
      ...
      Sut.SetupAsync<IAsyncRepository<BudgetsDataContext, IEntity>, Data.Budget>(r =>
    ...
    It should_save_a_new_Category_to_the_Data_Repository = () => {
      Sut.Verify<IAsyncRepository<BudgetsDataContext, IEntity>>(p => p.SaveAsync(Argument.IsAny<Data.Category>(), Argument.IsAny<CancellationToken>()), Times.Once());
    };
